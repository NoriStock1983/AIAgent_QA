...
class RAGRepository(RAGRepositoryInterface):
    ...
    def search(self, search_condition: SearchCondition) -> str:
        ...
        query_vector = self.create_embedding(search_condition.question.get_question())  # メソッド名を規約に合わせて変更
    ...
    def insert(self, insert_rag_data: InsertRagData):
        ...
        sub_category_name = insert_rag_data.get_sub_category_name()
        question = insert_rag_data.get_question()
        contents = insert_rag_data.get_contents()
        metadata = insert_rag_data.get_metadata()
        embedding = self.create_embedding(question)  # メソッド名を規約に合わせて変更
